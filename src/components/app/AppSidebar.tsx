...
import { currentCompany } from "@/lib/mock";
...
const launchpadItems = [
  ...
  { to: "/app/launchpad/kill-my-idea", label: "Kill My Idea" },
  { to: "/app/launchpad/funding-score", label: "Funding Score" },
  { to: "/app/launchpad/first-10-customers", label: "First 10 Customers" },
  { to: "/app/launchpad/business-plan", label: "Business Plan" },
  { to: "/app/launchpad/investor-emails", label: "Investor Emails" },
  { to: "/app/launchpad/idea-vs-idea", label: "Idea vs Idea" },
  { to: "/app/launchpad/landing-page", label: "Landing Page" },
  ...
  return (
    <aside className="hidden lg:flex w-60 shrink-0 flex-col border-r border-sidebar-border bg-sidebar text-sidebar-foreground">
      <div className="flex h-14 items-center gap-2 px-4 border-b border-sidebar-border">
        ...
        <div className="font-display text-sm font-semibold tracking-tight">LaunchpadNOVA</div>
      </div>

      <nav className="flex-1 overflow-y-auto px-2 py-3 text-sm">
        <SidebarLink to="/app/dashboard" icon={LayoutDashboard} active={isActive("/app/dashboard")}>
          Dashboard
        </SidebarLink>
      ...
      <div className="border-t border-sidebar-border p-3">
        <div className="rounded-md border border-sidebar-border bg-sidebar-accent/40 p-3">
          <div className="text-[10px] uppercase tracking-wider text-muted-foreground">You're at</div>
          <div className="mt-0.5 text-sm font-medium">{currentCompany.stage}</div>
          <div className="mt-0.5 text-xs text-muted-foreground">Next: build GTM</div>
        </div>
      </div>
    </aside>
  );
...
function SidebarSection({
  icon: Icon, label, color, items, activePath,
}: {
  icon: React.ComponentType<{ className?: string }>;
  label: string;
  color: string;
  ...
  const sectionActive = items.some((i) => activePath.startsWith(i.to.split("/").slice(0, 4).join("/")) && activePath.includes(items[0].to.split("/")[2]));
  return (
    <div className="mt-3">
