...
import { Search, Bell, Command, ChevronsUpDown, LogOut } from "lucide-react";
...
export function AppTopbar() {
  ...
  const initials = (profile?.full_name || user?.email || "U").split(" ").map((n) => n[0]).slice(0, 2).join("").toUpperCase();
  ...
  return (
    <header className="sticky top-0 z-30 flex h-14 items-center gap-3 border-b border-border bg-background/80 px-4 backdrop-blur">
      ...
      <button onClick={() => setOpenOperator(true)} className="ml-auto flex w-full max-w-md items-center gap-2 rounded-md border border-border bg-card px-3 py-1.5 text-left text-sm text-muted-foreground hover:bg-accent">
